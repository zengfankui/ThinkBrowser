...
BrowserApplication::BrowserApplication(int &argc, char **argv)
    ...
    , m_localServer(0)
{
    QCoreApplication::setOrganizationName(QLatin1String("ITHINK"));
    QCoreApplication::setApplicationName(QString("THINK"));
    QCoreApplication::setApplicationVersion(QLatin1String("V0.0.0.1"));
#ifdef Q_WS_QWS
    ...
    QMessageBox::information(0, tr("iThink"),
                 tr("当前系统不支持 OpenSSL， 使用SSL的页面将不可用。"));
    }
#endif
    ...
    // 使用应用程序翻译文件
    installTranslator(QLatin1String("iThink_") + localSysName);
...
void BrowserApplication::quitBrowser()
{
    ...
    if (tabCount > 1) {
        int ret = QMessageBox::warning(mainWindow(), QString(),
                           tr("当前有 %1 个窗口以及 %2 个标签已经打开，\n"
                              "你确定要退出吗?").arg(m_mainWindows.count()).arg(tabCount),
...
void BrowserApplication::postLaunch()
{
    ...
    setWindowIcon(QIcon(QLatin1String(":mainwindowicon.svg")));
