...
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
    QFile historyFile(QDesktopServices::storageLocation(QDesktopServices::DataLocation)+ QLatin1String("/history"));
#else
    QFile historyFile(QStandardPaths::writableLocation(QStandardPaths::DataLocation)+ QLatin1String("/history"));
#endif
...
void HistoryMenu::postPopulated()
{
    ...
    QAction *showAllAction = new QAction(tr("显示全部历史记录"), this);
    connect(showAllAction, SIGNAL(triggered()), this, SLOT(showHistoryDialog()));
    addAction(showAllAction);

    QAction *clearAction = new QAction(tr("清除历史记录"), this);
