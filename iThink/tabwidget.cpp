...
{
    QMenu menu;
    menu.addAction(tr("新建标签(&T) "), this, SIGNAL(newTab()), QKeySequence::AddTab);
    int index = tabAt(position);
    if (-1 != index) {
        QAction *action = menu.addAction(tr("克隆标签"),
                this, SLOT(cloneTab()));
        action->setData(index);

        menu.addSeparator();

        action = menu.addAction(tr("关闭本标签(&C)"),
                this, SLOT(closeTab()), QKeySequence::Close);
        action->setData(index);

        action = menu.addAction(tr("关闭其他标签(&O)"),
                this, SLOT(closeOtherTabs()));
        action->setData(index);

        menu.addSeparator();

        action = menu.addAction(tr("重新装载标签"),
                this, SLOT(reloadTab()), QKeySequence::Refresh);
        action->setData(index);
    } else {
        menu.addSeparator();
    }
    menu.addAction(tr("重新装载所有标签"), this, SIGNAL(reloadAllTabs()));
    ...
    , m_tabBar(new TabBar(this))
{
    ...
    // Actions
    m_newTabAction = new QAction(QIcon(QLatin1String(":addtab.png")), tr("新建标签(&T)"), this);
    m_newTabAction->setShortcuts(QKeySequence::AddTab);
    m_newTabAction->setIconVisibleInMenu(false);
    connect(m_newTabAction, SIGNAL(triggered()), this, SLOT(newTab()));

    m_closeTabAction = new QAction(QIcon(QLatin1String(":closetab.png")), tr("关闭标签(&C)"), this);
    m_closeTabAction->setShortcuts(QKeySequence::Close);
    m_closeTabAction->setIconVisibleInMenu(false);
    connect(m_closeTabAction, SIGNAL(triggered()), this, SLOT(closeTab()));

    m_nextTabAction = new QAction(tr("显示后一个标签"), this);
    ...
    m_previousTabAction = new QAction(tr("显示前一个标签"), this);
    shortcuts.clear();
    ...
    m_recentlyClosedTabsAction = new QAction(tr("最近关闭的标签"), this);
...
void TabWidget::closeTab(int index)
{
    ...
    if (WebView *tab = webView(index)) {
        if (tab->isModified()) {
            ...
            closeConfirmation.setWindowTitle(tr("关闭确认"));
            closeConfirmation.setInformativeText(tr("当前页面已经修改，关闭将丢失所有修改的内容！\n"
                                                     "你确定要关闭当前页吗?\n"));
