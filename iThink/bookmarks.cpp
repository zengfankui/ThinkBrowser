...
#define BOOKMARKBAR "书签栏"
#define BOOKMARKMENU "书签菜单"
...
#endif
    QString bookmarkFile = dir + QLatin1String("/bookmarks.xbm");
    if (!QFile::exists(bookmarkFile))
        bookmarkFile = QLatin1String(":defaultbookmarks.xbm");

    XbelReader reader;
    m_bookmarkRootNode = reader.read(bookmarkFile);
    if (reader.error() != QXmlStreamReader::NoError) {
        QMessageBox::warning(0, QLatin1String("装载书签"),
            tr("装入书签失败：第 %1行, 第 %2列:\n"
...
void BookmarksManager::save() const
{
    ...
    if (!writer.write(bookmarkFile, m_bookmarkRootNode))
        qWarning() << "书签管理器: 保存到文件失败：" << bookmarkFile;
}
...
{
    QString fileName = QFileDialog::getOpenFileName(0, tr("打开书签文件"),
                                                     QString(),
                                                     tr("书签 (*.xbm *.xml)"));
    if (fileName.isEmpty())
        return;
    ...
    if (reader.error() != QXmlStreamReader::NoError) {
        QMessageBox::warning(0, QLatin1String("装载书签"),
            tr("装载书签失败，第 %1 行, 第 %2 列:\n"
               "%3").arg(reader.lineNumber()).arg(reader.columnNumber()).arg(reader.errorString()));
    }

    importRootNode->setType(BookmarkNode::Folder);
    importRootNode->title = (tr("已经装载 %1").arg(QDate::currentDate().toString(Qt::SystemLocaleShortDate)));
    addBookmark(menu(), importRootNode);
}

void BookmarksManager::exportBookmarks()
{
    QString fileName = QFileDialog::getSaveFileName(0, tr("保存书签"),
                                tr("%1 ibroser.xbm").arg(QCoreApplication::applicationName()),
                                tr("书签 (*.xbm *.xml)"));
    if (fileName.isEmpty())
        return;
...
{
    setText(BookmarksManager::tr("插入书签"));
    m_node = node;
}
    ...
    , m_node(node)
{
    if (m_title) {
        ...
        setText(BookmarksManager::tr("名称修改"));
    } else {
        m_oldValue = m_node->url;
        setText(BookmarksManager::tr("地址修改"));
    }
}
...
#define MIMETYPE QLatin1String("application/bookmarks.xbm")
     ...
     Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
    ...
    undoStack->beginMacro(QLatin1String("移动书签"));
...
void BookmarksDialog::customContextMenuRequested(const QPoint &pos)
{
    ...
    if (index.isValid() && !tree->model()->hasChildren(index)) {
        menu.addAction(tr("打开"), this, SLOT(open()));
        menu.addSeparator();
    }
    menu.addAction(tr("删出"), tree, SLOT(removeOne()));
